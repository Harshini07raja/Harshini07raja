<div align="center">

<img src="https://capsule-render.vercel.app/api?type=waving&color=gradient&customColorList=6,11,20&height=220&section=header&text=Harshini%20Bharathi%20R&fontSize=42&fontColor=ffffff&animation=fadeIn&fontAlignY=38&desc=Full%20Stack%20Developer%20%7C%20AI%2FML%20Enthusiast%20%7C%20CSE%20Undergrad&descAlignY=58&descAlign=50" width="100%"/>

<a href="https://git.io/typing-svg">
  <img src="https://readme-typing-svg.demolab.com/?lines=Building+scalable+full-stack+products;React.js+%7C+Spring+Boot+%7C+MongoDB;Always+shipping%2C+always+learning;Open+to+SDE+Internships+2026&font=Fira%20Code&center=true&width=600&height=50&duration=3000&pause=1000&color=A78BFA&vCenter=true&size=22"/>
</a>

<br/>

<img src="https://img.shields.io/badge/CSE%20Undergraduate-SSN%20College%20of%20Engineering-7C3AED?style=for-the-badge&logo=academia&logoColor=white"/>
<img src="https://img.shields.io/badge/CGPA-7.81-A78BFA?style=for-the-badge"/>
<img src="https://img.shields.io/badge/Location-Chennai%2C%20India-6D28D9?style=for-the-badge&logo=googlemaps&logoColor=white"/>

<br/><br/>


<a href="https://www.linkedin.com/in/harshini-bharathi-r">
  <img src="https://img.shields.io/badge/LinkedIn-7C3AED?style=for-the-badge&logo=linkedin&logoColor=white"/>
</a>
<a href="mailto:bharathiharshini07@gmail.com">
  <img src="https://img.shields.io/badge/Email-6D28D9?style=for-the-badge&logo=gmail&logoColor=white"/>
</a>
<a href="https://github.com/Harshini07raja">
  <img src="https://img.shields.io/badge/GitHub-5B21B6?style=for-the-badge&logo=github&logoColor=white"/>
</a>

<br/><br/>

<img src="https://komarev.com/ghpvc/?username=YOUR_USERNAME&style=for-the-badge&color=8b5cf6&label=PROFILE+VIEWS"/>
<img src="https://img.shields.io/github/followers/YOUR_USERNAME?style=for-the-badge&color=a78bfa&label=FOLLOWERS&logo=github"/>
<img src="https://img.shields.io/github/stars/YOUR_USERNAME?style=for-the-badge&color=7c3aed&label=STARS&logo=github"/>

</div>

<br/>

## 🟣 About Me

<img align="right" width="35%" src="https://user-images.githubusercontent.com/74038190/212284158-e840e285-664b-44d7-b79b-e264b5e54825.gif"/>

```yaml
profile:
  name: "Harshini Bharathi R"
  role: "Aspiring Full-Stack Software Engineer"
  current:
    - "B.E. Computer Science @ SSN College of Engineering"
    - "Building production-style full-stack systems"
  focus:
    - "Scalable web applications with React.js + Spring Boot"
    - "RESTful API design & secure authentication systems"
    - "MongoDB / SQL data modeling for real-world apps"
    - "Exploring Agentic AI & Information Retrieval"
  mindset: "Ship clean, ship working, ship with purpose"
```

I'm a Computer Science undergraduate who enjoys turning ideas into working, end-to-end products — from designing the database schema to shipping a polished UI. My recent work spans **insurance claim management systems** and **full-stack e-commerce platforms**, with a growing interest in **AI/ML and agentic systems** after attending the TIER 2026 workshop on Text Analytics, Information Retrieval, and Agentic AI.

I care about writing code that's clean, scalable, and actually solves a problem — not just code that runs.

### 🎯 Open To
- 💼 **Software Engineering Internships (SDE / Full-Stack)**
- 🤝 **Open Source Collaboration** on web & AI/ML projects
- 🌱 **Learning opportunities** in Cloud, DevOps & AI-driven products
- 💬 **Tech discussions** on React, Spring Boot, and system design

<br/>

## 🟣 Tech Stack

<div align="center">

### Languages
<img src="https://skillicons.dev/icons?i=java,python,c,cpp,ts&theme=dark"/>

### Frontend
<img src="https://skillicons.dev/icons?i=react,html,css,tailwind,javascript&theme=dark"/>

### Backend & Databases
<img src="https://skillicons.dev/icons?i=spring,nodejs,mongodb,mysql,oracle&theme=dark"/>

### Cloud, DevOps & Tools
<img src="https://skillicons.dev/icons?i=git,github,vscode,postman,figma&theme=dark"/>

</div>

<br/>

## 🟣 Featured Projects

<details open>
<summary><b>🛒 E-Commerce Web Application</b> — React.js · Spring Boot · MongoDB</summary>
<br/>

> Full-stack e-commerce platform with product catalog, cart, order processing, and a role-based admin dashboard.

- 🔐 Implemented **JWT-based authentication** with role-based access control for users and admins
- ⚙️ Designed **RESTful APIs** in Spring Boot for product, user, and order management
- 🗄️ Integrated **MongoDB** for efficient storage and retrieval of product and user data
- 📊 Built an **admin dashboard** for managing products, pricing, stock, and images

`React.js` `Spring Boot` `MongoDB` `JWT` `REST API`

</details>

<details>
<summary><b>📋 Insurance Claim Processing System</b> — Spring Boot · MySQL</summary>
<br/>

> Backend system for end-to-end insurance claim submission, verification, and approval workflows.

- 🏗️ Designed and implemented the **claim lifecycle**: submission → verification → approval
- ⚙️ Built backend services in **Spring Boot** to manage claim requests and user roles
- 🗄️ Structured a **relational database schema** for efficient policy and claim data storage/retrieval

`Spring Boot` `MySQL` `REST API` `Role Management`

</details>

<br/>

## 🟣 GitHub Analytics

<div align="center">

<img height="165" src="https://github-readme-stats.vercel.app/api?username=Harshini07raja&show_icons=true&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&icon_color=8B5CF6&text_color=C9D1D9"/>
<img height="165" src="https://github-readme-streak-stats.herokuapp.com/?user=Harshini07raja&theme=tokyonight&hide_border=true&background=0D1117&ring=8B5CF6&fire=A78BFA&currStreakLabel=A78BFA"/>

<br/>

<img width="48%" src="https://github-readme-stats.vercel.app/api/top-langs/?username=Harshini07raja&layout=compact&theme=tokyonight&hide_border=true&bg_color=0D1117&title_color=A78BFA&text_color=C9D1D9"/>

<br/><br/>

<img src="https://github-readme-activity-graph.vercel.app/graph?username=Harshini07raja&theme=tokyo-night&hide_border=true&bg_color=0D1117&color=A78BFA&line=8B5CF6&point=C9D1D9"/>

</div>

<br/>

